...
- ✏️ **手动修改**：支持手动编辑和调整旅行计划中的任何内容
- 🔄 **AI 智能修改**：根据用户反馈，AI 自动优化和调整旅行计划
...
- `PUT /travel/plans/{plan_id}` - 手动更新旅行计划
- `POST /travel/plans/{plan_id}/modify-with-ai` - 使用 AI 根据用户反馈修改旅行计划
...
**方式一：手动修改**
1. 在行程详情页面点击"编辑"按钮
2. 直接修改活动名称、时间、地点、描述、费用等信息
3. 点击"保存"按钮保存修改

**方式二：AI 智能修改**
1. 在行程详情页面找到"AI 修改"功能
2. 输入您的修改需求，例如："增加一个博物馆参观"、"将第三天改为自由活动"、"减少餐饮预算"
3. AI 会根据您的反馈自动调整旅行计划
4. 查看修改后的计划，确认后保存
